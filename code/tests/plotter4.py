...
def plot(mdb, kx, ky, xlabel='', ylabel='', scale=[1,1], invert=[True,False], colour=cc, folder='', K=0, loc='upper right', bbox=(1.05, 1.12)):
    ...
    # axp = []
    # for i in range(1,10):
    #     axp.append(fig.add_subplot(3, 3, i))
    fig, axp = plt.subplots(3, 3, figsize=(12, 12), sharey=False)
    # axp[0][0] = fig.add_subplot(331)
    # axp[0][1] = fig.add_subplot(332)
    # axp[0][2] = fig.add_subplot(333)
    # axp[1][0] = fig.add_subplot(334)
    # axp[1][1] = fig.add_subplot(335)
    # axp[1][2] = fig.add_subplot(336)
    # axp[2][0] = fig.add_subplot(337)
    # axp[2][1] = fig.add_subplot(338)
    # axp[2][2] = fig.add_subplot(339)
    fig2 = plt.figure(figsize=(12, 7))
    axo11 = fig2.add_subplot(221)
    axo12 = fig2.add_subplot(222)
    axo13 = fig2.add_subplot(223)
    axo14 = fig2.add_subplot(224)
    for oi in range(2):
                        ...
                        try:
                            try:
                                ...
                                if ui < 2:
                                    ...
                                    axp[2][0].scatter(x, y, color=cc[ti] if oi else lc(cc[ti]), label=testl[ti] if not ui and not ai and oi else None)
                                    # if not oi:
                                    #     axp[0][0].scatter(x, y, color=lc(cc[ui]))
                                    # else:
                                    #     axp[0][0].scatter(x, y, color=cc[ui], label=uenwl[ui] if not ti and not ai else None)
                                else:
                                    axp[0][1].scatter(x, y, color=cc[ui] if oi else lc(cc[ui]), label=uenwl[ui] if not ti and not ai and oi else None)
                                    axp[1][1].scatter(x, y, color=cc[ai] if oi else lc(cc[ai]), label=attl[ai] if not ti and not ui and oi else None)
                                    axp[2][1].scatter(x, y, color=cc[ti] if oi else lc(cc[ti]), label=testl[ti] if not ui and not ai and oi else None)
                                    # if not oi:
                                    #     axp[0][1].scatter(x, y, color=lc(cc[ui]))
                                    # else:
                                    #     axp[0][1].scatter(x, y, color=cc[ui], label=uenwl[ui] if not ti and not ai else None)
                                i3 = int(ui/2)
                                if not oi:
                                    axp[0][2].scatter(x, y, color=lc(cc_compare[i3]))
                                else:
                                    axp[0][2].scatter(x, y, color=cc_compare[i3], label=nwl[i3] if not ti and not ai and not ui % 2 else None)

                                if not oi:
                                    ecl = atk['ECL'][r]
                                    for xp, yp, e in zip(x, y, ecl):
                                        axp[2][2].scatter(xp, yp, color=cc[int(e)], label=('N/A' if e == 3 else 'ECL ' + str(int(e))) if not e in ecl_list else None, alpha=0.8)
                                        ecl_list.append(e)
                                    acounts += len(y)
                                else:
                                    if y.size:
                                ...
                                if oi:
                                    ...
                                    oiutcounts += len(y)
                                ms=80
                                axo11.scatter(x, y, color=cc[ai] if oi else lc(cc[ai]), s=ms)
                                axo12.scatter(x, y, color=cc[ui] if oi else lc(cc[ui]), s=ms)
                                axo13.scatter(x, y, color=cc[ti] if oi else lc(cc[ti]), s=ms)
                                if not oi:
                                    ecl = atk['ECL'][r]
                                    for xp, yp, e in zip(x, y, ecl):
                                        axo14.scatter(xp, yp, color=cc[int(e)], s=ms, alpha=0.8)
    ...
    axp[1][2].boxplot(ally)
    f1, f2, g1, g2 = axp[0][2].axis()
    axp[1][2].set_ylim(g1, g2)
    axp[1][2].set_xticklabels(('U-M', 'Q-M', 'U-V', 'Q-V'))

    axp[0][1].set_yticklabels([])
    axp[0][2].set_yticklabels([])
    axp[1][1].set_yticklabels([])
    axp[1][2].set_yticklabels([])
    axp[2][1].set_yticklabels([])
    axp[2][2].set_yticklabels([])
    ...
    axp[0][0].set_xlim(f1, f2)
    axp[0][0].set_ylim(g1, g2)
    axp[0][1].set_xlim(f1, f2)
    axp[0][1].set_ylim(g1, g2)
    axp[1][0].set_xlim(f1, f2)
    axp[1][0].set_ylim(g1, g2)
    axp[1][1].set_xlim(f1, f2)
    axp[1][1].set_ylim(g1, g2)
    axp[2][0].set_xlim(f1, f2)
    axp[2][0].set_ylim(g1, g2)
    axp[2][1].set_xlim(f1, f2)
    axp[2][1].set_ylim(g1, g2)
    ...
    axo11.add_artist(AnchoredText(str(outcounts) + '/' + str(oiutcounts) + '\nK=' + str(K), loc=2))
    axp[0][0].set_ylabel(ylabel)
    axp[1][0].set_ylabel(ylabel)
    axp[2][0].set_ylabel(ylabel)
    axo11.set_ylabel(ylabel)
    axo13.set_ylabel(ylabel)
    axp[0][0].set_xlabel('A. ')
    axp[0][1].set_xlabel('B. ')
    axp[0][2].set_xlabel('C. ')
    axp[1][0].set_xlabel('D. ')
    axp[1][1].set_xlabel('E. ')
    axp[1][2].set_xlabel('F. ')
    axp[2][0].set_xlabel('G. ' + xlabel)
    axp[2][1].set_xlabel('H. ' + xlabel)
    axp[2][2].set_xlabel('I. ' + xlabel)
    axo12.set_xlabel(xlabel)
    axo13.set_xlabel(xlabel)
    axo14.set_xlabel(xlabel)
    axp[0][0].legend(loc=loc, bbox_to_anchor=bbox) # 'best' (1.05, 1.12)
    axp[0][1].legend(loc=loc, bbox_to_anchor=bbox) # 'best'
    axp[0][2].legend(loc=loc, bbox_to_anchor=bbox) # 'best'
    axp[1][0].set_zorder(1)
    axp[2][0].set_zorder(1)
    axp[1][0].legend(loc=loc, bbox_to_anchor=(1.35, 1.12)) # 'best'
    axp[2][0].legend(loc=loc, bbox_to_anchor=(1.35, 1.12)) # 'best'
    axp[2][2].legend(loc=loc, bbox_to_anchor=bbox) # 'best'
    kx = '_'.join(kx.split())
    ky = '_'.join(ky.split())
